...
// Represents normalized tabular Row
...
func (t *tabular) padRow(arr []string, padding int) []string {
	if len(arr) < 1 {
		return arr
	}
...
func (t *tabular) padLeft(width int, str string) string {
	b := createBuffer()
...
func (t *tabular) padRight(width int, str string) string {
	b := createBuffer()
	b.Write(str, 1)
	b.Write(" ", (width - runewidth.StringWidth(str)))
	return b.String()
...
func (t *tabular) padCenter(width int, str string) string {
	b := createBuffer()
...
func (t *tabular) buildLine(padded_widths []int, padding []int, l Line) string {
...
func (t *tabular) buildRow(elements []string, padded_widths []int, paddings []int, d Row) string {
...
func (t *tabular) SetWrapDelimiter(r rune) {
	t.WrapDelimiter = r
}
...
func (t *tabular) SetSplitConcat(r string) {
	t.SplitConcat = r
}

// Render the data table
func (t *tabular) Render(format ...interface{}) string {
	var lines []string
...
func (t *tabular) getWidths(headers []string, data []*tabularRow) []int {
...
func (t *tabular) SetTitle(title ...string) *tabular {
...
func (t *tabular) SetHeaders(headers []string) *tabular {
	t.Headers = headers
	return t
}
...
func (t *tabular) SetFloatFormat(format byte) *tabular {
	t.FloatFormat = format
	return t
}

// Set Align Type, Available options: left, right, center
func (t *tabular) SetAlign(align string) {
	t.Align = align
}

// Select the padding function based on the align type
func (t *tabular) getAlignFunc() func(int, string) string {
...
func (t *tabular) SetEmptyString(empty string) {
	t.EmptyVar = empty + " "
}
...
func (t *tabular) SetHideLines(hide []string) {
	t.HideLines = hide
}

func (t *tabular) SetWrapStrings(wrap bool) {
	t.WrapStrings = wrap
}
...
func (t *tabular) SetMaxCellSize(max int) {
	t.MaxSize = max
}

// Sets dense mode
// Under dense mode, no space line between rows
func (t *tabular) SetDenseMode() {
	t.DenseMode = true
}

func (t *tabular) splitElement(e string) (bool, string) {
...
func (t *tabular) wrapCellData() []*tabularRow {
	var arr []*tabularRow
	var cleanSplit bool
	var addr int
	if len(t.Data) == 0 {
	...
	for index := 0; index <= len(t.Data); index++ {
		...
		if next.Continuos {
			arr = append(arr, next)
			next = &tabularRow{Elements: new_elements}
			index--
...
func Create(data interface{}) *tabular {
	t := &tabular{FloatFormat: 'f', MaxSize: 30}
