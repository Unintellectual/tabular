...
func createFromString(data [][]string) []*tabularRow {
	rows := make([]*tabularRow, len(data))

	for index, el := range data {
		rows[index] = &tabularRow{Elements: el}
	}
	return rows
}

// Create normalized array of rows from mixed data (interface{})
func createFromMixed(data [][]interface{}, format byte) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, element := range data {
		...
		rows[index_1] = &tabularRow{Elements: normalized}
	}
	return rows
}

// Create normalized array from ints
func createFromInt(data [][]int) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, arr := range data {
		...
		rows[index_1] = &tabularRow{Elements: row}
	}
	return rows
}

// Create normalized array from float64
func createFromFloat64(data [][]float64, format byte) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, arr := range data {
		row := make([]string, len(arr))
		for index, el := range arr {
...
func createFromInt32(data [][]int32) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, arr := range data {
		row := make([]string, len(arr))
		for index, el := range arr {
...
func createFromInt64(data [][]int64) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, arr := range data {
		row := make([]string, len(arr))
		for index, el := range arr {
...
func createFromBool(data [][]bool) []*tabularRow {
	rows := make([]*tabularRow, len(data))
	for index_1, arr := range data {
		row := make([]string, len(arr))
		for index, el := range arr {
...
func createFromMapMixed(data map[string][]interface{}, format byte) (headers []string, tData []*tabularRow) {
...
// Keys will be used as header
func createFromMapString(data map[string][]string) (headers []string, tData []*tabularRow) {
